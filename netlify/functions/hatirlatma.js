// PEKDOĞRU Filo — Otomatik hatırlatma fonksiyonu (Netlify Scheduled Function)
...
exports.handler = async () => {
  ...
  try{
    ...
    if(!satirlar.length){ ozet.mesaj = 'Yaklaşan/geçmiş belge yok; e-posta gerekmedi.'; return json(200, ozet); }

    // 3) Aynı hatırlatmayı tekrar gönderme (son 40 günde gönderildiyse atla)
    ...
    if(!yeni.length){ ozet.mesaj = 'Acil belge var ama tümüne yakın zamanda bildirim gönderilmiş.'; return json(200, ozet); }

    // 4) E-posta gönder (Resend API)
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: gonderen,
        to: [alici],
        subject: `📋 Filo hatırlatma: ${yeni.length} belge için işlem gerekli`,
        html,
      }),
    });
    if(!res.ok){
      const hata = await res.text();
      return json(200, {...ozet, ok:false, mesaj: 'E-posta gönderilemedi: '+hata.slice(0,200)});
    }

    // 5) Gönderim kaydını güncelle
    ...
    return json(200, ozet);
  }catch(e){
    return json(200, {...ozet, ok:false, mesaj: 'Hata: '+(e.message||e)});
  }
};
